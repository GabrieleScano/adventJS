...
// Tenemos que crear una función que nos pasa una cadena de 
// texto con símbolos y tenemos que transformarlo al 
// número correcto. ¡Ojo! Si encuentras un símbolo 
// que no entendemos, mejor que devolvamos un NaN:
...
const keys = { 
  ...
  const decodeNumbers = symbols =>  {
      return symbols.split('').reduceRight( ( result, symbol, index, array ) => {
          const currentValue = keys[symbol] || NaN
          return currentValue < keys[array[index + 1]]
          ? result - currentValue
          : result + currentValue
      }, 0)
  }
